...
#### OPDS书籍完整内容解析实现
- [ ] 问题分析：当前OPDS书籍只创建描述性章节，未解析真实内容
- [ ] 下载功能：实现从sourceUrl下载epub文件到临时目录
- [ ] 解析集成：在addBookFromOpds方法中集成EpubParser解析功能
- [ ] 章节创建：将解析出的真实章节保存到数据库
- [ ] 元数据更新：正确设置totalChapters等书籍信息
- [ ] 错误处理：添加下载失败和解析失败的降级处理
- [ ] 临时文件清理：确保下载的临时文件被正确清理
- [ ] 测试验证：验证OPDS书籍能显示完整的真实章节内容
