...
export function formatDate(dateText: string, dateFormat?: string): string {
  try {
    ...
    cleanedDateText = cleanedDateText
      .trim()
      .replace(/^[\s\-_:：.,，。、]+|[\s\-_:：.,，。、]+$/g, "");
    ...
    if (!dateFormat) {
      ...
      const relativeTimePatterns = [
        {
          regex: /(\d+)\s*分钟前/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "minute"),
        },
        {
          regex: /(\d+)\s*小时前/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "hour"),
        },
        {
          regex: /(\d+)\s*天前/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "day"),
        },
        {
          regex: /(\d+)\s*周前/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "week"),
        },
        {
          regex: /(\d+)\s*个月前/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "month"),
        },
        ...
        {
          regex: /(\d+)\s*minute(?:s)?\s*ago/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "minute"),
        },
        {
          regex: /(\d+)\s*hour(?:s)?\s*ago/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "hour"),
        },
        {
          regex: /(\d+)\s*day(?:s)?\s*ago/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "day"),
        },
        {
          regex: /(\d+)\s*week(?:s)?\s*ago/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "week"),
        },
        {
          regex: /(\d+)\s*month(?:s)?\s*ago/,
          handler: (matches: RegExpMatchArray) => dayjs().subtract(parseInt(matches[1]), "month"),
        },
    ...
    try {
      ...
      logger.warn(
        `指定格式日期解析失败: ${cleanedDateText}, 格式: ${dateFormat}, 错误: ${(error as Error).message}`
      );
    }
    return dateText;
  } catch (error) {
