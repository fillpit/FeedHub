...
export class GitUploadService {
  ...
  async uploadToGit(
    ...
    try {
      ...
      logger.info(
        `[GitUploadService] 开始上传到Git仓库: ${options.gitUrl}, 分支: ${options.gitBranch}`
      );
   ...
   * 切换到指定分支
   */
  private async switchToBranch(git: SimpleGit, tempDir: string, branch: string): Promise<void> {
    ...
    try {
      ...
      const branchExists =
        branches.all.includes(branch) || branches.all.includes(`origin/${branch}`);
  ...
  async validateGitConfig(options: GitConfig): Promise<ApiResponseData<void>> {
    try {
      ...
      if (options.authType === "https") {
        ...
        logger.info(
          `[GitUploadService] Token认证验证通过，Token长度: ${options.token?.length || 0}`
        );
      } else {
