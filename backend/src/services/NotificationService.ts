...
export class NotificationService {
  ...
  constructor(@inject(TYPES.SettingService) settingService: SettingService) {
   ...
   * 发送订阅更新错误通知
   */
  async sendFeedUpdateErrorNotification(
    userId: string,
    feedTitle: string,
    error: string
  ): Promise<void> {
   ...
   * 发送新订阅内容通知
   */
  async sendNewFeedItemsNotification(
    userId: string,
    feedTitle: string,
   ...
   * 发送动态路由错误通知
   */
  async sendDynamicRouteErrorNotification(
    userId: string,
    routePath: string,
    error: string
  ): Promise<void> {
