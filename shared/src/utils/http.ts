...
import { HTTP_STATUS, ERROR_CODES, RETRY_CONFIG } from "../constants";
...
export function createTimeoutPromise(timeout: number): Promise<never> {
  return new Promise((_, reject) => {
    setTimeout(() => {
      reject(
        new AppError(`请求超时 (${timeout}ms)`, ERROR_CODES.NETWORK_ERROR, HTTP_STATUS.BAD_REQUEST)
      );
    }, timeout);
  });
}
...
export async function parseResponseData(
  ...
  try {
    ...
    throw new AppError("响应数据解析失败", ERROR_CODES.NETWORK_ERROR, HTTP_STATUS.BAD_REQUEST);
  }
}
