<template>
  <div class="package-management">
      ...
      <el-table :data="filteredPackages" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="info" size="small" @click="showPackageDetails(row)"> 详情 </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <!-- 安装包对话框 -->
    <el-dialog v-model="showInstallDialog" title="安装新包" width="500px">
      <el-form :model="installForm" :rules="installRules" ref="installFormRef">
        <el-form-item label="包名" prop="packageName">
          <el-input v-model="installForm.packageName" placeholder="请输入npm包名，如: lodash" />
        </el-form-item>
        <el-form-item label="版本" prop="version">
          <el-input v-model="installForm.version" placeholder="留空安装最新版本" />
        </el-form-item>
      </el-form>
    ...
    <!-- 包详情对话框 -->
    <el-dialog v-model="showDetailsDialog" title="包详情" width="800px">
      <div v-if="selectedPackage" class="package-details">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="大小">{{
            selectedPackage.size ? formatSize(selectedPackage.size) : "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="使用次数">{{
            selectedPackage.usageCount || 0
          }}</el-descriptions-item>
          <el-descriptions-item label="安装时间">{{
            selectedPackage.installedAt ? formatDate(selectedPackage.installedAt) : "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="最后使用">{{
            selectedPackage.lastUsedAt ? formatDate(selectedPackage.lastUsedAt) : "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="安装路径" :span="2">{{
            selectedPackage.installPath || "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="依赖" :span="2">
            <div v-if="selectedPackage.dependencies && selectedPackage.dependencies.length > 0">
              <el-tag
                v-for="dep in selectedPackage.dependencies"
                :key="dep"
                size="small"
...
const loadStats = async () => {
  try {
    ...
    if (response.data) {
      ...
      const computedInstalledPackages = packages.value.filter(
      ...
      const computedTotalUsage = packages.value.reduce(
        (sum, pkg) => sum + (pkg.usageCount || 0),
        0
      );
      ...
      stats.value = {
        ...
        totalSize: typeof d.totalSize === "number" ? d.totalSize : computedTotalSize,
        totalUsage: typeof d.totalUsage === "number" ? d.totalUsage : computedTotalUsage,
      };
    }
  } catch (error) {
...
const confirmUninstall = async (pkg: NpmPackage) => {
  try {
    await ElMessageBox.confirm(`确定要卸载包 "${pkg.name}" 吗？此操作不可恢复。`, "确认卸载", {
