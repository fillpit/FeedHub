<template>
  <el-dialog v-model="visible" title="初始化脚本" width="600px" :before-close="handleClose">
    <div class="script-init-content">
      ...
      <div v-if="initType === 'template'" class="init-option">
        ...
        <div class="template-description">
          <p v-if="selectedTemplate === 'basic'" class="template-desc">
            基础模版：只包含 package.json 和入口文件 main.js
          </p>
          <p v-if="selectedTemplate === 'complex'" class="template-desc">
            复杂模版：在基础模版基础上增加 utils 工具目录和工具脚本
          </p>
        </div>
      </div>
      ...
      <div v-if="initType === 'git'" class="init-option">
        ...
        <el-input
          v-model="gitSubPath"
          placeholder="子目录路径（可选，如：scripts/rss）"
          class="git-input"
        />
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="handleInit" :loading="loading"> 初始化 </el-button>
      </div>
    </template>
  </el-dialog>
</template>
