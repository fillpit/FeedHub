...
interface LightpandaSwitchProps {
  readonly enabled: boolean;
  readonly isSaving: boolean;
  readonly onToggle: () => void;
}

interface LightpandaInputProps {
...
interface LightpandaTestResultProps {
  readonly result: TestResultData | null;
}

interface LightpandaSaveActionsProps {
  readonly isSaving: boolean;
...
interface LightpandaConfigProps {
  readonly onStateChange: () => void;
}

/**
 * Switch button row for turning Lightpanda on/off
 */
const LightpandaSwitch: React.FC<LightpandaSwitchProps> = ({ enabled, isSaving, onToggle }) => (
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl bg-app-bg border border-app-border gap-4 shadow-sm">
    <div className="space-y-0.5">
      <span className="text-xs font-bold text-tx-primary">激活 Lightpanda 全文渲染代理</span>
      <p className="text-[11px] text-tx-tertiary leading-relaxed">
        开启后，文章解析引擎在抓取网页时将优先连接 Lightpanda。它的抓取优先级仅次于 CloakBrowser，但高于普通 Chrome CDP。
      </p>
    </div>
    <button
      onClick={onToggle}
 ...
 */
const LightpandaInput: React.FC<LightpandaInputProps> = ({ debugUrl, testLoading, onChange, onTest }) => (
 ...
 */
const LightpandaTestResult: React.FC<LightpandaTestResultProps> = ({ result }) => {
 ...
 */
const LightpandaSaveActions: React.FC<LightpandaSaveActionsProps> = ({ isSaving, saveSuccess, saveError, onSave }) => (
...
export const LightpandaConfig: React.FC<LightpandaConfigProps> = ({ onStateChange }) => {
  const [enabled, setEnabled] = useState(false);
  const [debugUrl, setDebugUrl] = useState("http://localhost:9222");
  ...
  useEffect(() => {
    ...
    api.getSiteSettings()
      .then((data) => {
        setEnabled(data.lightpanda_enabled === "1");
        setDebugUrl(data.lightpanda_url || "http://localhost:9222");
      })
      .catch((err) => {
        console.error("Failed to load Lightpanda settings", err);
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  const handleSave = async (nextEnabled: boolean) => {
    ...
    try {
      await api.updateSiteSettings({
        lightpanda_enabled: nextEnabled ? "1" : "0",
        lightpanda_url: debugUrl,
      });
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const res = await api.testLightpandaConnection({ lightpanda_url: debugUrl });
      setTestResult(res);
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <LightpandaSwitch enabled={enabled} isSaving={isSaving} onToggle={() => handleSave(!enabled)} />
      <LightpandaInput debugUrl={debugUrl} testLoading={testLoading} onChange={setDebugUrl} onTest={handleTest} />
      <LightpandaTestResult result={testResult} />
      <LightpandaSaveActions isSaving={isSaving} saveSuccess={saveSuccess} saveError={saveError} onSave={() => handleSave(enabled)} />
    </div>
  );
};
