...
interface ChromeSwitchProps {
  readonly enabled: boolean;
  readonly isSaving: boolean;
  readonly onToggle: () => void;
}

interface ChromeInputProps {
...
interface ChromeTestResultProps {
  readonly result: TestResultData | null;
}

interface ChromeSaveActionsProps {
  readonly isSaving: boolean;
...
interface ChromeDebuggingConfigProps {
  readonly onStateChange: () => void;
}

/**
 * Switch button row for turning Chrome debugging on/off
 */
const ChromeSwitch: React.FC<ChromeSwitchProps> = ({ enabled, isSaving, onToggle }) => (
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl bg-app-bg border border-app-border gap-4 shadow-sm">
    <div className="space-y-0.5">
      <span className="text-xs font-bold text-tx-primary">激活全文渲染代理</span>
      <p className="text-[11px] text-tx-tertiary leading-relaxed">
        开启后，文章解析引擎在遭遇防爬拦截或动态脚本时将秘密唤醒此 Chrome 进行会话级别的完美解析。
      </p>
    </div>
    <button
      onClick={onToggle}
 ...
 */
const ChromeInput: React.FC<ChromeInputProps> = ({ debugUrl, testLoading, onChange, onTest }) => (
 ...
 */
const ChromeTestResult: React.FC<ChromeTestResultProps> = ({ result }) => {
 ...
 */
const ChromeSaveActions: React.FC<ChromeSaveActionsProps> = ({ isSaving, saveSuccess, saveError, onSave }) => (
...
export const ChromeDebuggingConfig: React.FC<ChromeDebuggingConfigProps> = ({ onStateChange }) => {
  const [enabled, setEnabled] = useState(false);
  const [debugUrl, setDebugUrl] = useState("http://127.0.0.1:9222");
  ...
  useEffect(() => {
    ...
    api.getSiteSettings()
      .then((data) => {
        setEnabled(data.cdp_enabled === "1");
        setDebugUrl(data.cdp_url || "http://127.0.0.1:9222");
      })
      .catch((err) => {
        console.error("Failed to load chrome settings", err);
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <ChromeSwitch enabled={enabled} isSaving={isSaving} onToggle={() => handleSave(!enabled)} />
      <ChromeInput debugUrl={debugUrl} testLoading={testLoading} onChange={setDebugUrl} onTest={handleTest} />
      <ChromeTestResult result={testResult} />
      <ChromeSaveActions isSaving={isSaving} saveSuccess={saveSuccess} saveError={saveError} onSave={() => handleSave(enabled)} />
    </div>
  );
};
