    ...
    <div class="debug-result-container">
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="执行结果" name="result">
          <el-table border v-if="Array.isArray(result?.result)" :data="result?.result" size="small" style="width: 100%">
            <el-table-column prop="image" label="封面" align="center" width="220" show-overflow-tooltip>
              <template #default="scope">
                <el-image :src="scope.row.image" fit="contain" style="height: 100%" />
              </template>
            </el-table-column>
            <el-table-column prop="title" label="标题" align="center" width="220" show-overflow-tooltip />
            <el-table-column prop="link" label="链接" align="center" show-overflow-tooltip />
            <el-table-column prop="pubDate" label="发布时间" width="200" align="center" show-overflow-tooltip />
            <el-table-column prop="author" label="作者" width="100" align="center" show-overflow-tooltip />
          </el-table>
        ...
        <el-tab-pane label="日志输出" name="logs">
          ...
          <div class="logs-panel">
            <div v-for="(log, index) in filteredLogs" :key="index" class="log-item" :class="getLogClass(log)">{{ log }}</div>
          </div>
        </el-tab-pane>
...
watch(() => props.visible, v => { if (v) activeTab.value = 'result'; });
</script>
