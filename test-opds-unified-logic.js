...
async function testOpdsUnifiedLogic() {
  ...
  try {
    ...
    const opdsConfigs = configsResponse.data.data.filter(config => 
      config.book && config.book.sourceType === 'opds'
    );
    ...
    // 2. 测试每个OPDS书籍的章节数据
    for (const config of opdsConfigs.slice(0, 2)) { // 只测试前2个
      console.log(`\n--- 测试书籍: ${config.book.title} ---`);
      
      // 检查书籍的章节记录
      console.log('检查数据库中的章节记录...');
      const chaptersResponse = await axios.get(`http://localhost:8008/api/book-rss/books/${config.book.id}/chapters`);
      ...
      if (jsonData.items && jsonData.items.length > 0) {
        ...
        console.log(`  - 内容预览: ${firstItem.content_text ? firstItem.content_text.substring(0, 100) : 'N/A'}...`);
      }
    }
